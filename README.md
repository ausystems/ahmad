...
| `music.mp3` | The track the gramophone (beside the brown door) plays — click it in-game to start, use the volume button to mute. Swap the file to change the music; keep the same filename. Positional audio needs the site served over http(s) — on a plain double-clicked file it falls back to simple distance-based volume. |
